...
const goldenPath = resolve(testDirectory, "m4-spec-prompt-creator-prompt.golden.json");
...
describe("M4.4 spec-prompt-creator prompt golden contract", () => {
  it("matches the approved local spec-prompt-creator invoke, alias, inspect, and list payloads", async () => {
    const fetchSpy = vi.spyOn(globalThis, "fetch").mockImplementation(async () => {
      throw new Error("M4.4 spec-prompt-creator golden tests must not call network fetch.");
    });

    try {
      const actual = await buildSpecPromptCreatorGolden();
      const expected = JSON.parse(readFileSync(goldenPath, "utf8")) as unknown;

      expect(actual).toEqual(expected);
      expect(fetchSpy).not.toHaveBeenCalled();
      assertSpecPromptCreatorBoundaries(actual);
    } finally {
...
async function buildSpecPromptCreatorGolden(): Promise<Record<string, unknown>> {
  return withM4PromptClient(async (client) => ({
    invoke_spec_prompt_creator_success: normalizeToolResult(
      await client.callTool({
        name: INVOKE_PROMPT_LIBRARY_COMMAND_TOOL_NAME,
        arguments: { command: "spec-prompt-creator" },
      }),
    ),
    invoke_spec_creator_alias_success: normalizeToolResult(
      await client.callTool({
        name: INVOKE_PROMPT_LIBRARY_COMMAND_TOOL_NAME,
        arguments: { command: "spec-creator" },
      }),
    ),
    invoke_prompt_creator_alias_success: normalizeToolResult(
      await client.callTool({
        name: INVOKE_PROMPT_LIBRARY_COMMAND_TOOL_NAME,
        arguments: { command: "prompt-creator" },
      }),
    ),
    inspect_spec_prompt_creator_success: normalizeToolResult(
      await client.callTool({
        name: INSPECT_PROMPT_LIBRARY_COMMAND_TOOL_NAME,
        arguments: { command: "spec-prompt-creator" },
      }),
    ),
    inspect_spec_creator_alias_success: normalizeToolResult(
      await client.callTool({
        name: INSPECT_PROMPT_LIBRARY_COMMAND_TOOL_NAME,
        arguments: { command: "spec-creator" },
      }),
    ),
    inspect_prompt_creator_alias_success: normalizeToolResult(
      await client.callTool({
        name: INSPECT_PROMPT_LIBRARY_COMMAND_TOOL_NAME,
        arguments: { command: "prompt-creator" },
      }),
...
async function createM4RepoRoot(): Promise<string> {
  const repoRoot = await mkdtemp(join(tmpdir(), "ppl-m4-spec-prompt-creator-"));
  ...
  await copyFile(
    resolve(repositoryRoot, "prompts", "spec-prompt-creator.md"),
    join(promptsDirectory, "spec-prompt-creator.md"),
  );
  return repoRoot;
}
...
async function withClient<T>(server: McpServer, run: (client: Client) => Promise<T>): Promise<T> {
  const client = new Client({
    name: "m4-spec-prompt-creator-golden-test",
    version: "0.0.0",
  });
...
function assertSpecPromptCreatorBoundaries(golden: Record<string, unknown>): void {
  const invokeSuccess = asRecord(golden.invoke_spec_prompt_creator_success);
  const specCreatorAliasInvokeSuccess = asRecord(golden.invoke_spec_creator_alias_success);
  const promptCreatorAliasInvokeSuccess = asRecord(golden.invoke_prompt_creator_alias_success);
  const invocationPayload = asRecord(invokeSuccess.structuredContent);

  expect(specCreatorAliasInvokeSuccess.structuredContent).toEqual(invokeSuccess.structuredContent);
  expect(promptCreatorAliasInvokeSuccess.structuredContent).toEqual(
    invokeSuccess.structuredContent,
  );
  ...
  expect(commands.filter((command) => command.command === "spec-prompt-creator")).toHaveLength(1);
  ...
  expect(listSerialized).not.toContain("an ongoing chat mode");
  expect(listSerialized).not.toContain("The conversation itself is the working context");
}
