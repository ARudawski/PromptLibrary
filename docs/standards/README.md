...
- Slice 0 has passed with caveats, Slice 1 fixture-backed invocation is the approved baseline, Slice 2.1 approved the `PromptSource` boundary and fake source seam, Slice 2.2 approved the public GitHub source adapter, and Slice 2.3 approved runtime cache TTL basics.
- Current implementation is bounded to Slice 2.4 stale-while-revalidate and last-known-good cache behavior.
- Do not treat partial-valid/cold-failure cache policy, inspect/list tools, real prompt files, hosted deployment, or private/auth/DB behavior as implemented.
