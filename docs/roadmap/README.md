...
Slice 0 was accepted with caveats through recorded Linear gate evidence, Slice 1 fixture-backed invocation has been approved as the implementation baseline, and Slice 2.1 approved the `PromptSource` boundary and fake source seam. The current approved work is Slice 2.2: the public GitHub source adapter behind `PromptSource`.

## Current focus

This repository currently supports Slice 1 fixture-backed invocation plus the Slice 2.1 `PromptSource` boundary and fake source seam. Slice 2.2 adds the public GitHub source adapter as isolated infrastructure. Runtime cache behavior, inspect/list tools, real prompt files, and hosted deployment are still later-slice work.
