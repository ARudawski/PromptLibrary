...
Downstream exposure rule: M5.1 personal-use trial protocol and results log is the next allowed product lane after this PL-111 state-update PR is reviewed, merged, and closed out. PL-117 tracks non-blocking long-form documentation drift and does not block M5.1 planning. M5.2/M5.3/M5.4, M6 hosted deployment, hosted/private suites, auth/OAuth, DB, ChatGPT cache/admin/debug tools, prompt editing, draft management, semantic routing, workflow/session state, additional real prompt files, and broader runtime behavior remain blocked without an explicit coordinator path.
```
...
- Lower-priority long-form docs may still carry older M3, Slice 4.1, or fixture wording. PL-117 tracks the non-blocking refresh for README.md, prompts/README.md, docs/roadmap/README.md, docs/qa/test-strategy.md, and docs/tool-reference.md.
